...
var _skrypte$elm_hnpwa$Hnpwa$page = function (feed) {
	...
	var msg = function (_p14) {
		...
		var str = A2(
			...
			feed,
			{ctor: '_Tuple2', _0: _p15._0, _1: _p17});
		var _p16 = feed.page;
		if (_p16.ctor === 'SingleItem') {
			return _elm_lang$core$Native_Utils.eq(_p16._0, _p17.id) ? str : _skrypte$elm_hnpwa$Hnpwa$nolist;
		} else {
			return str;
		}
	};
	...
	var pushComment = F2(
		function (items, _p18) {
			...
			var i = A2(
				...
				_elm_lang$core$List$head(
					A2(
						...
						A2(_elm_lang$core$Dict$get, _p24.id, feed.index))));
			var siblings = A2(_elm_lang$core$Dict$get, _p24.id, feed.index);
			var indexOf = function (item) {
					...
					function (list, index) {
						...
						while (true) {
							...
							if (_p20.ctor === '[]') {
								...
								if (_elm_lang$core$Native_Utils.eq(_p20._0, item)) {
				...
				var s = A2(_elm_lang$core$Maybe$withDefault, _skrypte$elm_hnpwa$Hnpwa$nolist, siblings);
				return A2(
					_elm_lang$core$Maybe$withDefault,
					0,
					A2(indexOf_, s, 0));
			...
			var c = {
				parent: A2(parent, _p24.id, items),
				item: i,
				index: indexOf(i),
				id: i.id,
				kids: kids(i)
	...
	var knode = F2(
		function (items, _p25) {
			...
			return {
				ctor: '_Tuple2',
				_0: _elm_lang$core$Basics$toString(_p27),
				_1: A2(
					htmlContent,
					items,
					{ctor: '_Tuple2', _0: _p27, _1: _p26._1})
			};
		});
	var htmlContent = F2(
		function (items, _p28) {
		...
		var db = F2(
			function (parentId, comments) {
				...
				if (_p41.ctor === 'Success') {
					return A3(_elm_lang$core$Dict$insert, parentId, _p41._0, feed.index);
				} else {
					return feed.index;
				}
			});
			...
			default:
				if (_p42._0.ctor === 'Just') {
					...
					return {
						...
						_0: _elm_lang$core$Native_Utils.update(
							feed,
							{page: _p46, comments: _krisajenkins$remotedata$RemoteData$NotAsked, index: _elm_lang$core$Dict$empty}),
						_1: _skrypte$elm_hnpwa$Hnpwa$getPage(_p46)
					};
				} else {
					return A2(
						_elm_lang$core$Platform_Cmd_ops['!'],
...
var _skrypte$elm_hnpwa$Hnpwa$init = function (loc) {
	var _p47 = _skrypte$elm_hnpwa$Hnpwa$pathto(loc);
	if (_p47.ctor === 'Just') {
		switch (_p47._0.ctor) {
			case 'Blank':
				return {
			...
			default:
				return {
					...
					_1: _skrypte$elm_hnpwa$Hnpwa$getPage(
						_skrypte$elm_hnpwa$Hnpwa$SingleItem(_p47._0._0))
				};
		}
	} else {
